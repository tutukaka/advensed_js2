...
class ItemBasket extends  Item{
    ...
    render() {
        return `<div data-art="${this.id}" class="basket__item bxbb"> <img src="image/${this.img}" 
            alt="photo"><h3>${this.title}</h3><p>цена: ${this.price}р</p>
            <div class="button basket_button add">+</div><div class="quantity_out">${this.quantity}</div><div class="diminish button basket_button">-</div></div>` ;
...
document.querySelector('.catalog').addEventListener('click', (event) => {
        if (event.target.classList.contains('catalog_button')) {
            for (const key of items.items) {
                if (+event.target.parentNode.dataset.art === key.id) {
                    basket.checkItems(key);
                    basket.outResult();
